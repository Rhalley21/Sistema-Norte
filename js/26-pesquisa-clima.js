...
function calcularENPS(pesquisa){
  ...
  return { score, promotores, detratores, neutros: respostas.length - promotores - detratores, total: respostas.length };
}

function criarPesquisaClima(){
  if(!_climaTituloNovo.trim() || !_climaPerguntaNova.trim()){ showToast('Preencha o título e a pergunta.'); return; }
  state.pesquisasClima.push({
    id: uid(), titulo: _climaTituloNovo.trim(), pergunta: _climaPerguntaNova.trim(),
    tipo: 'eNPS', ativa: true, criadoEm: new Date().toISOString(), criadoPor: meuPerfilId, respostas: [],
  });
  registrarAuditoria('pesquisa_clima.criada', { titulo: _climaTituloNovo.trim() });
  _climaNovaAberta = false; _climaTituloNovo = '';
...
function pageClima(){
  ...
  return `
    ...
    <div class="card">
      <h3>Nova pesquisa</h3>
      ${!_climaNovaAberta ? `<button class="btn btn-primary" onclick="_climaNovaAberta=true; render();">+ Criar pesquisa</button>` : `
        <div class="field"><label>Título (interno, só a empresa vê)</label><input value="${_climaTituloNovo}" oninput="_climaTituloNovo=this.value;" placeholder="Ex: eNPS — 1º trimestre 2026"></div>
        <div class="field"><label>Pergunta (o que o colaborador vê)</label><textarea oninput="_climaPerguntaNova=this.value;">${_climaPerguntaNova}</textarea></div>
    ...
    ${pesquisasAtivas.length ? pesquisasAtivas.map(p=>{
      const jaRespondi = (p.respostas||[]).some(r=>r.perfilId===meuPerfilId);
      const enps = calcularENPS(p);
      return `
      <div class="card">
        <h3>${p.titulo} <small>${p.ativa?'Ativa':'Encerrada'}</small></h3>
        <p class="page-desc">${p.pergunta}</p>
        ${!souRHOuAdmin && !jaRespondi ? `
          <div class="field"><label>Nota (0 = nunca recomendaria, 10 = recomendaria com certeza)</label>
            <div style="display:flex;gap:4px;flex-wrap:wrap;">
              ${Array.from({length:11},(_,i)=>i).map(n=>`<button class="btn btn-sm ${n<=6?'':n<=8?'':''}" style="min-width:36px;${n<=6?'border-color:var(--iniciar);':n<=8?'border-color:var(--desenvolver);':'border-color:var(--alavancar);'}" onclick="responderPesquisaClima('${p.id}', ${n})">${n}</button>`).join('')}
            </div>
          </div>
        ...
        ${souRHOuAdmin ? `
          ...
          ${(p.respostas||[]).filter(r=>r.comentario).length ? `
            ...
            ${p.respostas.filter(r=>r.comentario).map(r=>`<div class="small-muted" style="padding:6px 0;border-top:1px solid var(--line);">"${r.comentario}" <span style="opacity:.6;">— nota ${r.nota}</span></div>`).join('')}
          ` : ''}
          <button class="btn btn-ghost btn-sm" style="margin-top:10px;" onclick="encerrarPesquisaClima('${p.id}')">Encerrar pesquisa</button>
        ` : ''}
      </div>`;
    }).join('') : '<div class="empty">Nenhuma pesquisa de clima ativa no momento.</div>'}
    ...
    <div class="card">
      ...
      <table><thead><tr><th>Título</th><th>Score eNPS</th><th>Respostas</th><th>Criada em</th></tr></thead><tbody>
        ${pesquisasEncerradas.map(p=>{ const e = calcularENPS(p); return `<tr><td>${p.titulo}</td><td>${e?e.score:'—'}</td><td>${e?e.total:0}</td><td class="small-muted">${new Date(p.criadoEm).toLocaleDateString('pt-BR')}</td></tr>`; }).join('')}
      </tbody></table>
    </div>
    ` : ''}
  `;
